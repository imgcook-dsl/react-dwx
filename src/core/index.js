const tailwindMap = require("./constants");
const { hsl, rgb } = require("d3-color");
...
module.exports = function(schema, option) {
    ...
    for (let key in style) {
      switch (key) {
        ...
        case "borderRadius":
          ...
          style[key] = (parseInt(style[key]) / 32).toFixed(2) + "rem";
          break;
      }
    }
  ...
  const generateRender = (schema) => {
    ...
    if (className) {
      ...
      Object.keys(style[className]).forEach((_styleKey) => {
        // replace string  A to -a B to -b C to -c
        const styleKey = _styleKey.replaceAll(/([A-Z])/g, (txt) => {
          return `-${txt.toLowerCase()}`;
        });
        const styleName = style[className][_styleKey];
        const atomClass =
          tailwindMap[styleKey] && tailwindMap[styleKey][styleName];
        // console.log(styleKey, styleName, '=>', atomClass)

        if (_styleKey === "color") {
          // 组件颜色处理（读取 -> 可变/约束 -> 输出）

          // == cv lib ==
          // const HEX2HSL = convert.hex.hsl
          // const HSL2HEX = convert.hsl.hex
          // const cvHSL = HEX2HSL(styleName);
          // cvHSL[0] += 20;
          // const cvHEX = HSL2HEX(cvHSL);
          // console.log(styleName, '=>', cvHEX);


          // == d3 lib ==
          const dsRGB = rgb(styleName);
          const dsHSL = hsl(styleName);
          dsHSL.h = 90;
          dsHSL.s = 0.8;
          const dsHEX = dsHSL.hex();
          console.log(styleName, '=>', dsRGB, dsHSL, dsHEX);
          
          const outputColor = dsHEX;

          styleObject[_styleKey] = outputColor 
          return
        }

        if (atomClass) {
          classList.push(atomClass.replace(/\./g, ""));
        } else {
          styleObject[_styleKey] = styleName;
        }
      });
    }
    ...
    const classString = className
      ? ` className="${classList.join(" ")}" style={${JSON.stringify(
          styleObject
        )}}`
      : "";
